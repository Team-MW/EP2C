...
test('healthy result requires all three application tables and leaks no records', async () => {
    const calls = [];
    const db = Object.fromEntries(['user', 'document', 'folder'].map(name => [name, {
    ...
    assert.deepEqual(calls, ['user', 'document', 'folder']);
});
